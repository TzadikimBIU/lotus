...
External languages use the same execution contract as custom languages. They can define `mode: "execute"` or `mode: "transpile"`, `highlightLanguage` for inherited source highlighting, `targetLanguage` for generated-source output, `preprocessors` for stable staged source transforms before execution, and `extractorMode`, `extractorExecutable`, `extractorArgs`, `transpileExecutable`, and `transpileArgs` if they need partial source extraction. `highlightLanguage` can target built-ins, another language-pack language, or a local custom language; Lotus follows chained `highlightLanguage` references and otherwise uses the target language id as the highlight class. The manifest acts as registry data (not copied to plugin settings), and the vault's enabled choices are persisted normally.
